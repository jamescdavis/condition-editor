...
type DbRecord < T > = T & { id: ID };

export interface DatabaseCollection<T = AnyAttrs> {
    insert<S extends T | T[]>(data: S): S extends T ? DbRecord<T> : Array<DbRecord<T>>;
    find<S extends ID | ID[]>(ids: S): S extends ID ? DbRecord<T> : Array<DbRecord<T>>;
    findBy(query: T): DbRecord<T>;
    where(query: T | ((r: DbRecord<T>) => boolean)): Array<DbRecord<T>>;
    update(attrs: T): Array<DbRecord<T>>;
    update(target: ID | T, attrs: T): Array<DbRecord<T>>;
    remove(target?: ID | T): void;
    firstOrCreate(query: T, attributesForCreate?: T): DbRecord<T>;
}
...
export type Database = DatabaseValues & DatabaseMethods;
...
export interface Trait<M, O extends TraitOptions<M> = {}> {
    extension: O;
    __isTrait__: true;
}
...
export class JSONAPISerializer {
    ...
    serialize(object: ModelInstance, request: Request): JSONAPI.SingleResourceDoc;
